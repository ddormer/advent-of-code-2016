...
input_data = '''bkwzkqsxq-tovvilokx-nozvyiwoxd-172[fstek]
...
def parse_input(data):
    _data, checksum = data.split('[')
    return {'key': ''.join(_data.split('-')[:-1]), 'checksum': checksum[:-1], 'id': _data.split('-')[-1]}



def comparitor(a, b):
    if a[1] == b[1]:
        return [-1, 1][ord(a[0]) < ord(b[0])]
    return [-1, 1][a[1] > b[1]]
...
def frequency(text):
    """
    Determine the frequency of letters and then sort them by
    frequency and then by alphabetical order
    """
    occurances = {}
    for letter in text:
        if occurances.get(letter, None) is None:
            occurances[letter] = 0
        occurances[letter] += 1
    return sorted(occurances.items(), cmp=comparitor, reverse=True)



def verify(text, checksum):
    """
    Verify the text against the checksum.
    """
    _frequency = frequency(text)
    for k, v in enumerate(checksum):
        if _frequency[k][0] != v:
            return False
    return True


positions = range(0, len(string.ascii_lowercase))
alpha_to_position = dict(zip(string.ascii_lowercase, positions))
positions_to_alpha = dict(zip(positions, string.ascii_lowercase))


def decrypt(text, rotate):
    decrypted = []
    for char in text:
        position = alpha_to_position[char]
        decrypted.append(positions_to_alpha[
            ((position + rotate) % len(positions))])
    return ''.join(decrypted), rotate
...
def solve_a(data):
    ...
    """
    return sum([int(item['id'])
                for item in data if verify(item['key'], item['checksum'])])
...
def solve_b(data):
    ...
    """
    return [decrypt(item['key'], int(item['id']))
            for item in data if verify(item['key'], item['checksum'])]



if __name__ == '__main__':
    parsed = map(parse_input, input_data.split('\n'))
    print solve_a(parsed)
    pprint(solve_b(parsed))
